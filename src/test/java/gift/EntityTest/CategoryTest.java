...
public class CategoryTest {
    ...
    @Test
    void creationTest(){
        Category category = new Category("식품", "#8123f3D", "식품 url", "식품 description");
        assertAll(
                ()->assertThat(category.getName()).isEqualTo("식품"),
                ()->assertThat(category.getColor()).isEqualTo("#8123f3D"),
    ...
    void updateTest(){
        Category category = new Category("식품", "#8123f3D", "식품 url", "식품 description");
        category.update("음료", "#7324d2b", "음료 url", "음료 description");

        assertAll(
                () -> assertThat(category.getName()).isEqualTo("음료"),
                () -> assertThat(category.getColor()).isEqualTo("#7324d2b"),
